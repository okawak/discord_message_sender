...
- **ボットトークン**
- **チャンネル**: DiscordチャンネルIDを追加します。チャンネル名は任意で、Obsidian側のサブフォルダ名に含まれます。
- **通知テンプレート**: 同期後にDiscordへ送るメッセージを変更できます。利用可能な変数は`{count}`、`{channelName}`、`{channelId}`です。

デフォルトでは、通常メッセージは`DiscordLogs/<チャンネル名>-<チャンネルID>/`、URLクリッピングは`DiscordClippings/<チャンネル名>-<チャンネルID>/`に保存されます。チャンネル名が未設定の場合は、チャンネルIDだけを使います。
