...
この文書は、Rust中心の構成と公式APIによる保守性を維持する前提で行った比較実験の記録です。「直接wasm-bindgenを使用していた時点の配布サイズ」以降の値は記録コミット`d774154ff1b550239a2f7a185c0f2e643088acf2`で追加された当時の測定値であり、現在値ではありません。

## Obsidian公式HTML変換APIへの移行

未完成だった独自の`html_to_markdown` crateを削除し、Obsidian公式の`sanitizeHTMLToDom`、`htmlToMarkdown`、`stringifyYaml`へ移行しました。TypeScriptには取得元URLを基準に`href`と`src`を絶対化する薄いアダプターだけを残しています。圧縮、外部WASM、実行時のバイナリ取得は使用していません。

2026-09-12に同じ作業ツリーとツールチェーンでproduction buildした結果です。

| 成果物 | 移行前 | 公式API移行後 | 削減量 |
| --- | ---: | ---: | ---: |
| `pkg/parse_message_bg.wasm` | 674,098 bytes | **258,856 bytes** | 415,242 bytes（61.6%） |
| `dist/main.js` | 926,208 bytes | **373,200 bytes** | 553,008 bytes（59.7%） |
...
| 成果物 | bytes | SHA-256 |
| --- | ---: | --- |
| 最適化後`pkg/parse_message_bg.wasm` | **672,300** | `573d7064b1bfa3ef3cf95ec417c3cef5f965baf9c68b9b45138759f847d074b4` |
| 非圧縮`dist/main.js` | **923,808** | `ca3ac0d383e2c15503a048800157a913283b1c26928598d53ded510b29ec347d` |

`main.js`の上限1,000,000 bytesに対する余裕は76,192 bytesでした。当時の検証件数はBunが12ファイル120件、RustがHTML変換crateの単体414件・追跡fixture 1件・メッセージ処理crate 22件・互換性4件・doc test 2件です。

以前の追記にあったWASM **890,363 bytes**は測定段階が明記されておらず、現在の最適化後成果物とは一致しないため、現在値としては使用しません。以下はRust 1.98.0で測定した過去の比較記録として保持します。
...
以下は公式APIへの移行前に調査した候補です。現在はObsidianのHTML変換APIを採用したため、比較記録として残します。
...
当時の採用後にRust324件・Bun103件、型検査、Biome、ネイティブとWASMのclippy、実配布バンドルのオフライン初期化テストを通過しました。日時の追加テストは負の年・年0・5桁以上の年、各月末、うるう秒について以前のChrono書式と比較しています。HTML変換は当時の独自実装に対する既存294件の単体テストと2件の統合テストで確認した記録であり、現在の公式API移行後の検証件数ではありません。実Obsidian UIの操作テストは行っていません。
