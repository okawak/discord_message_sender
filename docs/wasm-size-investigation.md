...
この文書は、Rust中心の構成、既存機能、標準ライブラリによる保守性を維持する前提で行った比較実験の記録です。「直接wasm-bindgenを使用していた時点の配布サイズ」以降の値は記録コミット`d774154ff1b550239a2f7a185c0f2e643088acf2`で追加された当時の測定値であり、現在値ではありません。
...
`main.js`の上限1,000,000 bytesに対する余裕は76,192 bytesです。現在の検証件数はBunが12ファイル120件、RustがHTML変換crateの単体414件・追跡fixture 1件・メッセージ処理crate 22件・互換性4件・doc test 2件です。
