...
/// Escapes literal text embedded in a Markdown link label or image description.
/// Literal source text is escaped before its parent renderer wraps it, so Markdown
/// markup deliberately generated by child renderers stays active.
pub fn escape_markdown_label_text(text: &str) -> Cow<'_, str> {
...
/// Escapes characters that can terminate or regroup an inline Markdown link
/// destination while preserving the URL value seen by a Markdown parser.
pub fn escape_markdown_link_destination(destination: &str) -> Cow<'_, str> {
...
mod tests {
    ...
    #[case(r"a]b *literal*", r"a\]b \*literal\*")]
    #[case(r"a\b [label] _text_ `code`", r"a\\b \[label\] \_text\_ \`code\`")]
    fn escapes_literal_markdown_link_label_text(#[case] input: &str, #[case] expected: &str) {
        assert_eq!(escape_markdown_label_text(input), expected);
    }

    #[rstest]
    #[case("https://example.com/path", "https://example.com/path")]
    #[case("https://example.com/a)b", r"https://example.com/a\)b")]
    #[case("https://example.com/(a)", r"https://example.com/\(a\)")]
    fn escapes_markdown_link_destinations(#[case] input: &str, #[case] expected: &str) {
        assert_eq!(escape_markdown_link_destination(input), expected);
    }
